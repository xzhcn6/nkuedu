...
<script id="id_table_coursetimelist" type="text/html">    
	...
	<tr>
		...
		<td>{{value.endTime}}</td>
	    <td><a href="<%=request.getContextPath()%>/admin/updateCourseTime?id={{value.id}}&courseId={{value.courseId}}">修改</a>
...
<p align="center"><strong><font size="5">编辑课程时间</font></strong></p>
	...
	<tr>
		...
		<td>&nbsp</td>
		<td>&nbsp</td>
		<td><a href="<%=request.getContextPath()%>/admin/addCourseTime?id=${course.id}&selectId=${course.selectId}&name=${course.name}">
			添加时间</a></td>
	</tr>
